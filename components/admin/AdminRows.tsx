...
export default function AdminRows({
  ...
  async function save(event?: FormEvent) {
    ...
    if (kind === "training" && !(payload.title ?? "").trim()) {
      setMessage("Program title is required.");
      return;
    }
    if ((kind === "pilot" || kind === "curriculum") && !((payload.id ?? "") as string).trim()) {
      setMessage(`A ${singular.toLowerCase()} needs an id.`);
      return;
    }
    setBusy(true);
    const response = await fetch("/api/admin/settings", {
  ...
  async function remove(id: string) {
    ...
    const response = await fetch(
      `/api/admin/settings?action=${encodeURIComponent(kind)}&id=${encodeURIComponent(id)}`,
      { method: "DELETE" }
    );
    if (response.ok) {
  ...
  const safe = (r: RowItem, fallback = "") =>
    (r as { title?: string }).title?.trim() || r.id || fallback;
  const filtered = rows;
  ...
  return (
    <section aria-label={`${singular}s`} className="min-w-0 border-t-2 border-ink bg-white p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-0">
          <h2 className="font-display text-xl font-bold">
            {caption || `${singular}s`} ({rows.length})
          </h2>
      ...
      {editing && (
        <form
          onSubmit={save}
          className="mt-4 grid gap-3 rounded-lg border border-line bg-surface p-4"
        >
          <h3 className="text-sm font-bold text-ink">
            {editing.id ? `Edit ${editing.id}` : `Add a ${singular.toLowerCase()}`}
          </h3>
          {kind === "training" && (
            <>
              ...
              <label className="text-sm font-bold">
                Audience
                <input
                  value={(editing as { audience?: string }).audience ?? ""}
              ...
              <label className="text-sm font-bold">
                Duration
                <input
                  value={(editing as { duration?: string }).duration ?? ""}
              ...
              <label className="text-sm font-bold">
                Outcome
                <textarea
                  value={(editing as { outcome?: string }).outcome ?? ""}
          ...
          {kind === "pilot" && (
            <>
              ...
              <label className="text-sm font-bold">
                Cost
                <input
                  value={(editing as { cost?: string }).cost ?? ""}
              ...
              <label className="text-sm font-bold">
                Note
                <input
                  value={(editing as { note?: string }).note ?? ""}
            ...
            <>
              <label className="text-sm font-bold">
                ...
                <input
                  ...
                  onChange={(e) => setEditing({ ...editing, ageBand: e.target.value })}
                  className={`mt-1 w-full ${inputClass}`}
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={busy}
              className="bg-gold px-4 py-2 text-xs font-black text-ink transition hover:bg-gold-dark disabled:opacity-60"
            >
              {busy ? "Saving..." : "Save"}
            </button>
            <button
              ...
              className="border border-line px-4 py-2 text-xs font-black text-ink transition hover:border-navy"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="mt-4 divide-y divide-line">
          ...
          <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 py-2">
            <span className="min-w-0 flex-1">
              <span className="block text-sm">
                <strong>{safe(r)}</strong>{" "}
                {kind === "curriculum" && r.ageBand && (
                  <span className="text-slate-400">({r.ageBand})</span>
                )}
              </span>
            </span>
            <span className="flex shrink-0 gap-2">
              ...
              <button
                ...
                disabled={busy}
                className="text-xs font-bold text-ink underline disabled:opacity-60"
              >
        ...
        {shown.length === 0 && (
          <p className="py-3 text-sm text-slate-500">No {singular.toLowerCase()}s yet.</p>
        )}
      </div>
            ...
            >
              ...
              {kind === "curriculum" && previewing.skills?.length ? (
                <ul className="mt-4 list-disc space-y-1 pl-5 text-sm text-slate-700">
                  {(previewing.skills as string[]).map((s, i) => (
                    <li key={i}>{s}</li>
                  ))}
                </ul>
              ) : (
                <p className="mt-4 text-sm leading-6 text-slate-600">
                  {(previewing as { description?: string }).description ||
                    (previewing as { note?: string }).note ||
                    (previewing.item ?? "")}
                </p>
              )}
              <div className="mt-5 flex items-center justify-between gap-3">
                <strong className="font-display text-lg text-ink">
                  {kind === "pilot" ? (previewing as { cost?: string }).cost : ""}
                </strong>
                <button
                  onClick={() => setPreviewing(null)}
