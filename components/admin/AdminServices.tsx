...
export default function AdminServices({ setMessage, canDelete }: Props) {
  ...
  async function saveServiceItem(event?: FormEvent) {
    ...
    if (!service.id || !service.name) {
      setMessage("Service needs at least an id and name.");
      return;
    }
    setBusy(true);
  ...
  async function toggleServiceVisibility(item: Service) {
    ...
    if (response.ok) {
      setServices((current) => current.map((s) => (s.id === item.id ? payload : s)));
      setMessage(item.active ? "Service hidden." : "Service shown.");
    }
  }
              ...
              <>
                {categories.length > 0 && (
                  <select
                    ...
                    className={`mt-3 w-full sm:w-48 ${inputClass}`}
                  >
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <span className="block line-clamp-2 text-sm">
                          ...
                          <span className="text-slate-400">{s.priceLabel}</span>
                        </span>
                        {!s.active && (
                      ...
                      <RowActions
                        actions={[
                          ...
                          {
                            label: s.active ? "Hide" : "Show",
                            tone: "ink",
                            onClick: () => void toggleServiceVisibility(s),
                          },
                          {
                            label: "Delete",
                            tone: "red",
          ...
          <form onSubmit={saveServiceItem} className={editorCard}>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="min-w-0 text-sm font-bold">
                ...
                <input
                  value={service.id}
                  onChange={(e) => updateService("id", e.target.value)}
              ...
              <label className="min-w-0 text-sm font-bold">
                ...
                <select
                  ...
                  className={`mt-2 w-full ${inputClass}`}
                >
                  {Array.from(new Set([service.category, ...categories])).map((cat) => (
                    <option key={cat} value={cat}>
                      {cat}
                    </option>
                  ))}
              ...
              <label className="min-w-0 text-sm font-bold">
                ...
                <input
                  ...
                  onChange={(e) => updateService("priceLabel", e.target.value)}
                  className={`mt-2 w-full ${inputClass}`}
              ...
              <label className="min-w-0 text-sm font-bold">
                ...
                <input
                  value={service.tag}
                  onChange={(e) => updateService("tag", e.target.value)}
                  className={`mt-2 w-full ${inputClass}`}
                  placeholder="Website, Fabrication, etc."
                />
              </label>
              <label className="min-w-0 text-sm font-bold">
                Sort Order
                <input
                  ...
                  onChange={(e) => updateService("sortOrder", Number(e.target.value))}
                  className={`mt-2 w-full ${inputClass}`}
                />
              </label>
              ...
              <label className="flex items-center gap-2 text-sm font-bold">
                <input
                  ...
                  onChange={(e) => updateService("active", e.target.checked)}
                  className="h-4 w-4"
                />{" "}
            ...
            <SaveBar>
              ...
              {service.id && (
                <button
                  type="button"
                  onClick={() => setService(emptyService)}
            ...
            >
              ...
              <div className="p-6">
                <p className="truncate text-xs font-black uppercase tracking-widest text-navy">
                  {previewService.tag || previewService.category}
                </p>
