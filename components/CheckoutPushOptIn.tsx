    ...
    getPushStatus()
      .then((status) => {
        if (
          ...
          window.localStorage.getItem("genum-push-optout") !== "1"
        ) {
          setState("ready");
        }
      })
  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-3 rounded-2xl border border-line bg-white p-4 text-sm animate-fade-in">
      ...
      <span className="text-slate-600">
        Want a push notification when your order status changes?
      </span>
      {state === "busy" ? (
        ...
        <>
          <button
            onClick={onTurnOn}
            className="rounded-lg bg-navy px-4 py-2 text-xs font-bold text-white transition hover:bg-navy-dark"
          >
            Turn on
          </button>
          <button
            onClick={() => {
              window.localStorage.setItem("genum-push-optout", "1");
              setState("hidden");
            }}
