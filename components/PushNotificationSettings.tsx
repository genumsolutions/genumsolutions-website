...
import { getPushStatus, subscribeToPush, unsubscribeFromPush, type PushStatus } from '../lib/web-push'
  ...
  return (
    <section className="rounded-2xl border border-line bg-white p-6 shadow-card sm:p-8">
      ...
      <p className="mt-2 text-sm text-slate-600">
        Get a push on this device when your order status changes (payment confirmed, shipped, cancelled) — even with GENUM closed.
      </p>
      <div className="mt-5 flex flex-wrap items-center gap-3">
        {status.subscribed ? (
          ...
          >
            {busy ? <Loader2 size={16} className="animate-spin" aria-hidden="true" /> : <BellOff size={16} aria-hidden="true" />}
          ...
          >
            {busy ? <Loader2 size={16} className="animate-spin" aria-hidden="true" /> : <Bell size={16} aria-hidden="true" />}
            Turn on notifications
          </button>
        )}
        {status.permission === 'denied' && !status.subscribed && (
          <span className="text-xs text-slate-500">Notifications are blocked for this site — enable them in your browser settings.</span>
        )}
        {message && <span role="status" className="text-sm font-bold text-emerald-700">{message}</span>}
