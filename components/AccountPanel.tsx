      ...
      <main className="grid-paper min-h-[70vh]">
        <div className="mx-auto max-w-5xl space-y-6 px-5 py-12 lg:px-8" aria-hidden="true">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="h-28 animate-pulse border border-line bg-white" />
            <div className="h-28 animate-pulse border border-line bg-white" />
            <div className="h-28 animate-pulse border border-line bg-white" />
          </div>
        </div>
    ...
    <main className="grid-paper min-h-[70vh]">
      <div className="mx-auto max-w-5xl space-y-6 px-5 py-12 lg:px-8">
        <header className="border-t-4 border-navy bg-white p-6 sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div className="flex items-center gap-4">
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-navy font-display text-lg font-black text-white">
                {initials(customer.name)}
              </span>
              <div>
                <p className="text-xs font-black uppercase tracking-[.25em] text-navy">
                  Customer account{customer.tier === "pro" ? " · Pro" : ""}
                </p>
            ...
            <button
              onClick={logout}
              className="rounded-lg border border-line px-4 py-2 text-sm font-semibold text-ink transition hover:border-red-300 hover:text-red-600"
            >
              Log out
            </button>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <a
            href="#orders"
            className="rounded-2xl border border-line bg-white p-5 shadow-card transition hover:border-navy"
          >
            <strong className="block font-display text-3xl font-bold text-ink">
              {orders.length}
            </strong>
          ...
          >
            ...
            <span className="mt-1 block text-sm font-semibold text-muted">
              Support message{customer.messages.length === 1 ? "" : "s"}
            </span>
          </Link>
        </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="truncate text-xs text-slate-500">
                        ...
                        {order.provider === "cod"
                          ? "Pay on delivery"
                          : order.provider === "esewa"
                            ? "eSewa"
                            : order.provider === "khalti"
                              ? "Khalti"
                              : order.provider}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-[11px] font-black uppercase tracking-wide ${statusStyles[order.status] || "bg-slate-100 text-slate-700"}`}
                      >
                        {order.status}
                      </span>
        ...
        <SectionCard title="Your details">
          ...
          <form onSubmit={saveProfile} className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-3 sm:col-span-2">
              <button
                disabled={busy}
                className="rounded-lg bg-gold px-5 py-3 text-sm font-bold text-ink shadow-sm transition hover:bg-gold-dark disabled:opacity-60"
              >
        ...
        <SectionCard title="Your messages">
          {customer.messages.length === 0 ? (
            <p className="mt-3 pb-2 text-sm text-slate-500">
              No messages yet.{" "}
              ...
              {customer.messages.map((item) => (
                <div key={item.createdAt} className="py-4 first:pt-0 last:pb-0">
                  ...
                  <p className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-wide ${item.status === "resolved" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}
                    >
                      {item.status}
                    </span>
                  </p>
