...
export default function HeaderSession() {
  ...
  if (!ready)
    return (
      <span
        className="h-10 w-10 animate-pulse rounded-full border border-line"
        aria-hidden="true"
      />
    );
  ...
  if (!user) {
    return (
      <Link
        href="/login"
        className="flex h-10 w-10 items-center justify-center rounded-full bg-navy text-white transition hover:bg-navy-dark sm:h-auto sm:w-auto sm:gap-1.5 sm:px-5 sm:py-2"
        aria-label="Sign in"
      >
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 z-50 mt-2 w-64 border border-line bg-white shadow-xl animate-scale-in"
        >
          ...
          <div className="p-1.5">
            <Link
              ...
              className="block px-3 py-2 text-sm font-bold text-ink hover:bg-mist"
            >
              Your account
            </Link>
            {isStaffRole(user.role) && (
            ...
            <button
              onClick={logout}
              role="menuitem"
              className="block w-full px-3 py-2 text-left text-sm font-bold text-red-600 hover:bg-red-50"
            >
              Log out
            </button>
          </div>
