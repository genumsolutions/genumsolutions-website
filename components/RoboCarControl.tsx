...
export default function RoboCarControl() {
  ...
  const options: TransportOptions = {
    onTelemetry: (t) => {
      ...
      if (t.mode) {
        ...
        const isSelfAck =
          lastAckedModeRef.current !== null && lastAckedModeRef.current === reportedMode;
  ...
  const handleConnectBt = async () => {
    ...
    try {
      ...
      setConnected(true);
      setStatus(
        NativeTransport.available() ? "Car connected via app" : "Car connected over Bluetooth"
      );
      await t.requestState();
    } catch (e) {
  ...
  const handleLeftJoy = (x: number, y: number) => {
    ...
    const d = is2wd1m ? (y < -0.25 ? "F" : y > 0.25 ? "B" : "S") : joyToDirection(x, y);
    applyDir(d);
  };
    ...
    <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-12 overflow-y-auto max-h-screen">
      <div className="max-w-2xl">
        ...
        <p className="mt-2 text-sm leading-6 text-muted lg:text-base">
          Choose a mode, connect a BLE or WiFi car, then drive with the two virtual joysticks and
          the Select / Back buttons — just like the physical remote and its OLED display.
        </p>
      </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-line bg-surface p-4">
            <p className="flex items-center gap-2 text-sm font-bold text-ink">
              <Bluetooth size={16} className="text-navy" />{" "}
              {NativeTransport.available() ? "Car (via app)" : "Bluetooth (BLE)"}
            </p>
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px] overflow-hidden max-h-[calc(100vh-200px)]">
        <div className="rounded-2xl border border-line bg-white p-5 shadow-card lg:p-6">
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {!connected && (
                <span className="text-xs font-bold text-muted">Connect to switch</span>
              )}
            </div>
          </div>
            ...
            {lock ? (
              <div className="mb-4 rounded-xl bg-sky px-4 py-3">
                <p className="text-sm font-bold text-navy">
                  {connected
                    ? "This mode is driven by its hand-held remote."
                    : "Connect a device to unlock controls."}
                </p>
              </div>
            ) : null}
          </div>
          ...
          <div
            className={`mt-6 border-t border-line pt-6 transition ${lock ? "pointer-events-none opacity-40" : ""}`}
          >
            ...
            {(mode.controls.includes("start-stop") || mode.controls.includes("tuning")) && (
              <div className="mt-6 flex flex-wrap items-center gap-3">
                <button
                  type="button"
                  onClick={() => {
                    void send(mode.token);
                    setDriveStatus(`${mode.name} running`);
                  }}
                ...
                <button
                  ...
                  onClick={() => {
                    void send("4WD4M");
                    setDriveStatus(`${mode.name} stopped`);
                  }}
              ...
              <div className="mt-6 rounded-xl border border-line bg-surface p-4">
                <p className="text-sm font-semibold text-ink">
                  {mode.id === "website-server"
                    ? "Open the car’s web page"
                    : "Car acts as a client"}
                </p>
                <p className="mt-1 text-xs leading-5 text-muted">
                  {mode.id === "website-server"
        ...
        <aside className="rounded-2xl border border-line bg-white p-5 shadow-card lg:self-start lg:p-6">
          <h3 className="text-xs font-black uppercase tracking-[.2em] text-navy">
            About this mode
          </h3>
...
function OledDisplay({
  ...
  const line3 =
    angle != null
      ...
      : (telemetryMode ? `MODE ${telemetryMode}` : connected ? "READY" : "PRESS SELECT").slice(
          0,
          20
        );
  return (
...
function Joystick({
  ...
  return (
    ...
    >
      <span
        className="absolute left-1/2 top-1/2 h-1 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-border"
        aria-hidden="true"
      />
      <span
