...
} from "../supabase/functions/push-order-status/webpush";
...
describe("webpush crypto core (W-3, no Firebase)", () => {
  it("encrypts a payload the subscription private key can decrypt (RFC 8291 roundtrip)", async () => {
    ...
    const encrypted = await encryptPayload(
      subscription.publicB64,
      subscription.authB64,
      '{"title":"Order paid","body":"See you soon","url":"/account"}'
    );
    const plaintext = await decryptAes128gcm(
      encrypted,
      subscription.privatePkcs8,
      subscription.authB64,
