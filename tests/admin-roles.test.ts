...
vi.mock('../lib/supabase/server', () => ({
  ...
  createClient: () => ({
    auth: { getUser: async () => ({ data: { user: state.hasUser ? { id: 'u1', email: 'staff@genum.test' } : null } }) },
    from: () => ({
      select: () => ({
        eq: () => ({
...
import { getCurrentAdmin, getCurrentUserRole, isAdminRequest, isOwnerRequest, isStaffRequest } from '../lib/admin'
import { isAdminRole, isOwnerRole, isStaffRole, isValidAdminRole, roleAtLeast, roleRank } from '../lib/roles'
