...
const COLOR_UTILITY = /\b(?:bg|text|border|divide|ring|from|via|to)-(?:white|black|(?:slate|gray|zinc|neutral|stone|red|orange|amber|yellow|lime|green|emerald|teal|cyan|sky|blue|indigo|violet|purple|fuchsia|pink|rose)-(?:50|100|200|300|400|500|600|700|800|900|950))\b/g
...
const DIM_SAFE = new Set([
  ...
  'bg-red-500', 'bg-amber-500', 'bg-emerald-500', // status dots on dark panels
  'bg-emerald-600', // CTA button on dark cards
  'text-emerald-300', 'text-emerald-400', // glow accents on the car-control deck (dark by design)
  'bg-slate-600', 'bg-slate-700', 'bg-slate-800', 'bg-slate-900', // 3D scene + panel darks
  'border-slate-700', 'ring-slate-700', // outlines on those dark panels
])
...
function hasDimOverride(utility: string): boolean {
  ...
  const direct = new RegExp(`html\\[data-theme='dim'\\][^{]*\\.${escaped.replace(/\\/g, '')}[^a-z0-9-]`)
  ...
  return new RegExp(`html\\[data-theme='dim'\\][^}]*${escaped}`).test(dimCss) || grouped.test(dimCss)
}
