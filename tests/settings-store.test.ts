...
describe("getManagedSettings", () => {
  it("maps snake_case rows onto the camelCase Managed* shapes", async () => {
    state.reads = {
      ...
      pilot_cost_lines: [
        { id: "c1", item: "Filament", cost: "NPR 500", note: "n", active: true, sort_order: 1 },
      ],
    ...
    expect(bundle.company).toMatchObject({
      name: "GENUM SOLUTIONS",
      shortName: "GENUM",
      country: "Nepal",
    });
    ...
    expect(bundle.pilotCostLines).toEqual([
      { id: "c1", item: "Filament", cost: "NPR 500", note: "n", active: true, sortOrder: 1 },
    ]);
    expect(bundle.curriculumHighlights).toEqual([
      { id: "cur1", ageBand: "Ages 8-11", items: ["Build", "Code"], active: true, sortOrder: 2 },
    ]);
  });
  ...
  it("defaults active to true and treats a JSON-string items column like an array", async () => {
    state.reads = {
      ...
      curriculum_highlights: [
        { id: "cur2", age_band: "B", items: '["x","y"]', active: true, sort_order: 0 },
      ],
    };
...
describe("save mutations (camelCase in, snake_case out)", () => {
  ...
  it("saveCompanyInfo writes the single company row and invalidates the public cache", async () => {
    await saveCompanyInfo({
      ...
      vatLabel: "PAN",
      description: "desc",
      whatsappNumber: "+977 986-1842 552",
    ...
    // C5: WhatsApp normalizes to digits-only; social URLs pass through.
    expect(state.upserts[0]!.row.whatsapp_number).toBe("9779861842552");
    expect(state.upserts[0]!.row.facebook_url).toBe("https://facebook.com/genum");
    expect(state.upserts[0]!.row.instagram_url).toBe("");
