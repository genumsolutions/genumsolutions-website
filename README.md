...
### App-version contract (sibling `genumsolutions-app` repo)

- The Android app (Expo) ships its own UI and shares only the Supabase **database** with this site, never the site layout.
- `/app` reads the **live** `release.json` manifest from the shared `app-releases` bucket (written only by `upload-release.mjs` when a real APK is published), so it never advertises a version whose APK isn't ready.
- `lib/company.ts` → `androidApp` is the **bundled fallback** shown when the manifest is unreachable; it is regenerated by `node scripts/sync-app-fallback.mjs` (idempotent, CI-gated to stay typecheck-clean — never hand-edit).
- Version pins that must move **together**: `mobile/app.json` (`version` + `versionCode`) ↔ `mobile/src/config/site.ts` (`APP_VERSION`) ↔ git tag. Current: **3.2.0 / 53**.
