...
// C5 (2026-09-23): the payment-confirmation UI, extracted verbatim from the
// old client page so checkout/success/page.tsx can be a server component
// (it needs the shared company row for the WhatsApp nudge + socials).
export default function SuccessPanel() {
  ...
  return (
    <section className="max-w-lg rounded-2xl border border-line bg-white p-8 text-center shadow-sm">
      ...
      {(sessionId || provider) && state === "received" && (
        <p className="mt-2 text-xs text-slate-400">
          Payment verification is still processing. Your order is saved and will update shortly.
        </p>
      )}
      <div className="mt-7 flex flex-wrap justify-center gap-3">
        <Link
          href="/account"
          className="rounded-full bg-navy px-5 py-3 text-sm font-black text-white"
        >
          View my orders
        </Link>
        <Link
          href="/products"
          className="rounded-full border border-line px-5 py-3 text-sm font-black text-ink"
        >
          Keep exploring
        </Link>
      </div>
