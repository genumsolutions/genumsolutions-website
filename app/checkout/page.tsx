...
export default function CheckoutPage() {
  ...
  function validate() {
    if (!customer.name.trim() || !customer.email.trim() || !customer.address.trim()) {
      setMessage("Add your name, email, and delivery address to continue.");
      return false;
    }
    if (!account) {
      setMessage("Sign in to place your order - your build list is saved automatically.");
      return false;
    }
    if (!items.length) {
      setMessage("Your build list has no purchasable items.");
      return false;
    }
    return true;
  }
  ...
  async function payNepal(provider: "esewa" | "khalti") {
    if (!validate()) return;
    setBusy(true);
    setMessage(`Preparing ${provider === "esewa" ? "eSewa" : "Khalti"} payment...`);
    try {
      ...
      if (provider === "esewa" && result.action && result.fields) {
        clear();
        submitGatewayForm(result.action, result.fields);
        return;
      }
      setMessage(result.error || `${provider} is not configured yet.`);
      setBusy(false);
    } catch {
      setMessage(
        `${provider === "esewa" ? "eSewa" : "Khalti"} payment could not be started. Try again.`
      );
      setBusy(false);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-mist">
      <header className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 overflow-hidden px-5 py-3 lg:px-8 lg:py-4">
          <Link
            href="/"
      ...
      <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
        ...
        <h1 className="mt-3 font-display text-2xl font-bold tracking-tight sm:text-3xl lg:text-4xl">
          Ready to build?
        </h1>
        {!account && hydrated && lines.length > 0 && (
          <div className="mt-6 rounded-2xl border border-navy/20 bg-navy-light p-5 text-sm text-ink">
            <strong>Sign in to place your order.</strong> Your build list is already saved and will
            merge into your account.{" "}
            <Link href="/login" className="font-bold text-navy transition hover:text-navy-dark">
        ...
        {hydrated && lines.length > 0 && (
          <div className="mt-4 rounded-2xl border border-line bg-white p-4 text-sm animate-fade-in">
            <p className="text-xs font-black uppercase tracking-[.24em] text-navy">Newsletter</p>
            {/* C4 (2026-09-23): opt-in beside the order flow — consent checkbox required. */}
            <NewsletterOptIn variant="checkout" />
          </div>
        )}
        {!hydrated ? (
          <div
            className="mt-10 animate-pulse rounded-2xl border border-line bg-white p-12 text-center text-sm font-bold text-muted"
            role="status"
          >
            Loading your build list...
          </div>
          ...
          <div className="mt-10 rounded-2xl border border-dashed border-line bg-white p-12 text-center">
            <h2 className="font-display text-xl font-bold">
              We couldn&rsquo;t load your build list details.
            </h2>
            <p className="mt-2 text-sm text-muted">
              Check your connection and try again - your items are saved.
            </p>
            <Link
              href="/products"
              className="mt-5 inline-block rounded-lg bg-navy px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-navy-dark"
            >
              Back to products
            </Link>
          </div>
          ...
          <div className="mt-10 rounded-2xl border border-dashed border-gold bg-white p-8 text-center">
            <h2 className="font-display text-xl font-bold">
              Some items in your build list are no longer available.
            </h2>
            <p className="mt-2 text-sm text-muted">
              They were removed because they are out of stock or no longer listed.
            </p>
            <Link
              href="/products"
              className="mt-5 inline-block rounded-lg bg-navy px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-navy-dark"
          ...
          <div className="mt-8 grid gap-8 sm:mt-10 sm:gap-10 lg:grid-cols-[1.1fr_.9fr]">
                ...
                {items.map(({ product, quantity }) => (
                  ...
                  >
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          changeQuantity(product.id, Math.min(quantity + 1, product.stock))
                        }
              ...
              <div className="rounded-2xl border-t-4 border-navy bg-white p-6 shadow-card">
                <dl className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <dd className="font-bold">{formatNPR(total)}</dd>
                  </div>
                  <div className="flex items-center justify-between border-t border-line pt-3 text-lg font-black">
                    <dt>Total</dt>
                ...
                <div className="mt-5 hidden gap-3 lg:grid">
                  <button
                    onClick={() => payNepal("esewa")}
                    disabled={busy}
                    className="rounded-lg bg-navy px-5 py-3.5 text-sm font-bold text-white shadow-sm transition hover:bg-navy-dark disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Pay with eSewa
                  </button>
                  <button
                    onClick={() => payNepal("khalti")}
                    disabled={busy}
                    className="rounded-lg bg-navy px-5 py-3.5 text-sm font-bold text-white shadow-sm transition hover:bg-navy-dark disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Pay with Khalti
                  ...
                  <button
                    onClick={payCod}
                    disabled={busy}
                    className="rounded-lg border-2 border-dashed border-line px-5 py-3.5 text-sm font-bold text-muted transition hover:border-ink hover:text-ink disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    Reserve order · Pay on delivery
                  </button>
                </div>
                <div role="status" aria-live="polite">
                  {message && <p className="mt-4 text-sm font-bold text-gold">{message}</p>}
                </div>
              </div>
              ...
              <div className="sticky bottom-4 z-30 rounded-2xl border border-line bg-white p-4 shadow-card lg:hidden">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button
                    onClick={() => payNepal("esewa")}
                    disabled={busy}
                    className="rounded-lg bg-navy px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-navy-dark disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    eSewa
                  </button>
                  <button
                ...
                <button
                  onClick={payCod}
                  disabled={busy}
                  className="mt-2 w-full rounded-lg border-2 border-dashed border-line px-4 py-2.5 text-sm font-bold text-muted transition hover:border-ink hover:text-ink disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Pay on delivery
                </button>
              </div>
            ...
            <section
              ...
              className="rounded-2xl border border-line bg-white p-6 lg:order-first"
            >
              <h2 id="delivery-heading" className="font-display text-2xl font-bold">
                Delivery details
              </h2>
              <form
              ...
              <p className="mt-4 text-xs leading-5 text-muted">
                We confirm every order by email before dispatch.
              </p>
            </section>
          </div>
        )}
