...
const AdminPanel = dynamicImport(() => import("../../components/AdminPanel"), {
  ssr: false,
  loading: () => (
    <div className="mx-auto max-w-7xl px-5 py-12 text-sm text-muted">Loading admin…</div>
  ),
});
      ...
      <header className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-5 lg:px-8">
          <div>
            ...
            >
              ...
              <span className="leading-none">
                <strong className="block font-display text-lg font-bold tracking-tight text-ink sm:text-[22px]">
                  GENUM
                </strong>
                <span className="mt-1 block text-[9px] font-bold uppercase tracking-[0.32em] text-navy sm:text-[10px]">
          ...
          <div className="flex items-center gap-3">
            <span
              className="hidden max-w-[14rem] truncate rounded-full bg-navy-light px-4 py-2 text-xs font-bold text-navy sm:block"
              title={admin.email}
            >
              {admin.email}
            </span>
