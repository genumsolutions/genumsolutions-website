    ...
    switch (body.action) {
      case "company": {
        ...
        await saveCompanyInfo({
          ...
          // C5: socials + WhatsApp ride the same save (strings, '' = unused).
      ...
      case "pilot": {
        ...
        if (!l.item?.trim())
          return NextResponse.json({ error: "Cost line item is required." }, { status: 400 });
        const id = (l.id ?? "").trim() || `cost-${Date.now()}`;
        await savePilotCostLine({
          id,
          item: l.item.trim(),
          cost: String(l.cost ?? "").trim(),
      ...
      case "curriculum": {
        ...
        if (!h.ageBand?.trim())
          return NextResponse.json({ error: "Curriculum age band is required." }, { status: 400 });
        const id = (h.id ?? "").trim() || `curriculum-${Date.now()}`;
        await saveCurriculumHighlight({
          id,
          ageBand: h.ageBand.trim(),
  ...
  try {
    ...
    switch (action) {
      ...
      case "curriculum":
        ...
        await logActivity({
          action: "settings.deleted",
          entityType: "curriculum_highlights",
          entityId: id,
        });
        break;
      default:
