...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: profiles } = await db
      .from("profiles")
      .select("id, name, phone, address, role, tier")
      .in("id", ids);
...
export async function PATCH(request: Request) {
  ...
  if (role === "owner") {
    return NextResponse.json(
      { error: "The owner role cannot be assigned via the API." },
      { status: 400 }
    );
  }
...
export async function DELETE(request: Request) {
  if (!(await isOwnerRequest()))
    return NextResponse.json({ error: "Only the owner can delete users." }, { status: 403 });
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY)
    return NextResponse.json({ error: "Service role key is not configured." }, { status: 503 });
  const body = await request.json().catch(() => null);
  const userId = String(body?.userId || "");
...
export async function POST(request: Request) {
  ...
  if (action === "upsertRobotSetting") {
    ...
    const ok = await upsertUserRobotSettings(
      userId,
      robotId,
      robotName,
    ...
    if (!ok)
      return NextResponse.json({ error: "Could not save the robot settings." }, { status: 500 });
    await logActivity({
      action: "user.robot_settings_updated",
      entityType: "user",
      entityId: userId,
      details: { robotId, via: "admin" },
    });
    return NextResponse.json({ ok: true });
  }
  if (action === "deleteRobotSetting") {
    if (!(await isAdminRequest()))
      return NextResponse.json(
        { error: "Only admins can delete robot profiles." },
        { status: 403 }
      );
    const robotId = String(body?.robotId || "").trim();
    ...
    if (!ok)
      return NextResponse.json({ error: "Could not delete the robot settings." }, { status: 500 });
    await logActivity({
      action: "user.robot_settings_deleted",
      entityType: "user",
      entityId: userId,
      details: { robotId, via: "admin" },
    });
