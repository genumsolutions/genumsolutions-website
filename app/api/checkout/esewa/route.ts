...
export async function POST(request: Request) {
  ...
  if (!cart)
    return NextResponse.json(
      { error: "No purchasable items in your build list." },
      { status: 400 }
    );
  ...
  if (!order)
    return NextResponse.json(
      { error: "Could not save the order before payment." },
      { status: 500 }
    );
  ...
  try {
    ...
    await logTransaction({
      ...
      userId: user.id,
      provider: "esewa",
      amountNpr: cart.totalNpr,
      status: "failed",
      rawPayload: { error: "signing-failed" },
    });
    return NextResponse.json(
      { error: "Payment could not be started. Try again." },
      { status: 500 }
    );
  }
  ...
  await logTransaction({
    ...
    amountNpr: cart.totalNpr,
    status: "initiated",
    rawPayload: { transactionUuid: order.id },
  });
