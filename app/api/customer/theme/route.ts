...
export async function PUT(request: Request) {
  const user = await getSessionUser();
  if (!user)
    return NextResponse.json({ error: "Sign in to save your theme preference." }, { status: 401 });
  ...
  if (!saved)
    return NextResponse.json({ error: "Could not save your theme preference." }, { status: 500 });
