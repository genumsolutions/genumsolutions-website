...
function sanitizeSettings(value: unknown): Record<string, unknown> | null {
  ...
  for (const [key, raw] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (raw == null || ['string', 'number', 'boolean'].includes(typeof raw)) {
      out[key] = raw
    } else if (Array.isArray(raw) && raw.length <= 100 && raw.every((item) => ['string', 'number', 'boolean'].includes(typeof item))) {
      out[key] = raw
    }
...
export async function PUT(request: Request) {
  ...
  if (theme) {
    const { error } = await db.from('profiles').update({ theme_preference: theme }).eq('id', user.id)
    if (error) return NextResponse.json({ error: 'Could not save the theme preference.' }, { status: 500 })
    themeTouched = true
  }
  ...
  if ('settings' in body) {
    ...
    if (!settings) return NextResponse.json({ error: 'settings must be an object of simple values.' }, { status: 400 })
