...
async function markPaidAndLog(
  ...
  // paid/fulfilled - the transaction ledger row is still written for audit.
  try {
...
<form id="esewa-form" method="POST" action="${actionUrl}">
    ...
    if (action === "return") {
      ...
      if (!order || order.provider !== "esewa") {
        return html(
          `<!doctype html><html><body><script>location.replace('genumsolutions://checkout?provider=esewa&status=no-order')</script></body></html>`
        );
      }
      if (order.status === "paid") {
        return html(
          `<!doctype html><html><body><script>location.replace('genumsolutions://checkout/success?provider=esewa&order=${txnUuid}&paid=1')</script></body></html>`
        );
      }
      if (Number(payload.total_amount) !== order.total_npr) {
        return html(
          `<!doctype html><html><body><script>location.replace('genumsolutions://checkout?provider=esewa&status=amount-mismatch')</script></body></html>`
        );
      }
      const complete = await esewaStatusComplete(txnUuid, order.total_npr);
      if (!complete) {
        return html(
          `<!doctype html><html><body><script>location.replace('genumsolutions://checkout?provider=esewa&status=not-paid')</script></body></html>`
        );
      }
      await markPaidAndLog(order, String(payload.transaction_code || txnUuid), {
        ...payload,
    ...
    if (action === "cancel") {
      const orderId = url.searchParams.get("order") || "";
      return html(
        `<!doctype html><html><body><script>location.replace('genumsolutions://checkout?provider=esewa&order=${orderId}&status=cancelled')</script></body></html>`
      );
    }
