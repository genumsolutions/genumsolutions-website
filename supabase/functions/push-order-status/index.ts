...
function buildMessage(order: { status: string; items: unknown }): {
  title: string;
  body: string;
  url: string;
} {
...
async function pushExpo(
  ...
  if (!expoAccessToken) {
    return {
      ...
      skipped: 0,
      pruned: 0,
      reason: "EXPO_ACCESS_TOKEN not configured (Firebase dormant)",
    };
  }
  ...
  return {
    sent,
    skipped: (tokens ?? []).length - sent - deadTokens.length,
