...
export async function encryptPayload(
  ...
  const userKey = await crypto.subtle.importKey('raw', uaPublic as BufferSource, { name: 'ECDH', namedCurve: 'P-256' }, false, [])
  ...
  const ikm = await hkdf(
    uaAuth,
    ecdhSecret,
    concatBytes(new TextEncoder().encode('WebPush: info'), new Uint8Array([0x00]), uaPublic, ephemeralPublicRaw),
    32,
  )
  ...
  const sealed = new Uint8Array(
    await crypto.subtle.encrypt({ name: 'AES-GCM', iv: nonce as BufferSource, tagLength: 128 }, key, record as BufferSource),
  )
...
export async function createVapidAuthorization(endpoint: string, publicKeyB64: string, privateKeyB64: string): Promise<string> {
  ...
  const key = await crypto.subtle.importKey('pkcs8', privateKeyDer as BufferSource, { name: 'ECDSA', namedCurve: 'P-256' }, false, ['sign'])
  const signature = new Uint8Array(
    await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, key, new TextEncoder().encode(unsigned) as BufferSource),
  )
  if (signature.length !== 64) throw new Error(`Unexpected ECDSA signature length: ${signature.length}`)
