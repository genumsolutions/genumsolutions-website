...
function makerworldImages(design: Record<string, unknown>): string[] {
  const instances = Array.isArray(design.instances) ? (design.instances as Record<string, unknown>[]) : []
...
function makerworldSpecs(design: Record<string, unknown>): string[] {
  ...
  if (typeof best.materialCnt === 'number' && best.materialCnt > 0) specs.push(`Materials: ${best.materialCnt}`)
  if (Array.isArray(best.instanceFilaments)) specs.push(`Filament types: ${(best.instanceFilaments as unknown[]).length}`)
  ...
  if (comp.devProductName) specs.push(`Compatible: ${comp.devProductName} · ${comp.nozzleDiameter ?? 0.4}mm nozzle`)
...
async function extractMakerWorld(url: string): Promise<Preview> {
  ...
  const categories = Array.isArray(design.categories) ? (design.categories as Record<string, unknown>[]) : []
...
function scanJsonLd(html: string): { name?: string; image?: string; description?: string; price?: string; siteName?: string; specs?: string[] } {
  ...
  for (const block of blocks) {
    ...
    const scan = (node: Record<string, unknown>): { name?: string; image?: string; description?: string; price?: string; siteName?: string; specs?: string[] } | null => {
...
async function runCreate(body: Record<string, unknown>, client: ReturnType<typeof createClient>) {
  ...
  const rawSpecs = Array.isArray(overrides?.specs) ? (overrides.specs as unknown[]) : (preview.specs ?? [])
