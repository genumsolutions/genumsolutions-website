...
serve(async (req) => {
  ...
  try {
    ...
    // If succeeded, also move the order to paid. C1 (2026-09-23): the status
    // flip + product-stock decrement are one atomic, row-locked, idempotent
    // RPC (mark_order_paid) shared with the confirm routes and edge payments,
    // so a webhook + redirect race can never double-decrement.
