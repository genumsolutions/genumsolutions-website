...
async function callerRole(req: Request): Promise<{ role: string | null; client: ReturnType<typeof createClient>; error?: string }> {
  ...
  if (!callerToken) return { role: null, client: null as unknown as ReturnType<typeof createClient>, error: 'Sign in to manage services.' }
  ...
  if (userError || !callerId) return { role: null, client: adminClient, error: 'Sign in to manage services.' }
  ...
  const { data: callerProfile } = await adminClient
    ...
    .eq('id', callerId)
    .maybeSingle()
  if (!callerProfile?.role) return { role: null, client: adminClient, error: 'Only staff members can manage services.' }
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'toggle') {
      ...
      const { data, error: toggleError } = await client.from('services').update({ active: !current }).eq('id', id).select()
