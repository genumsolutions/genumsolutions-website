    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onerror = () => {
        this.options.onStatus?.(
          "error",
          "Could not reach the car. Check its IP and that WiFi car firmware is running."
        );
...
declare global {
  interface Window {
    ...
    __GENUM_ROBO__?: {
      ...
      ingress: (
        kind: "telemetry" | "status" | "connected" | "disconnected" | "error",
        payload: string
      ) => void;
    };
  }
...
export class NativeTransport implements CarTransport {
  ...
  static available(): boolean {
    return (
      typeof window !== "undefined" && !!window.ReactNativeWebView && !!(window as Window).GENUM_APP
    );
  }
  ...
  private emit(kind: string, payload: unknown) {
    try {
      this.bridge?.postMessage(JSON.stringify({ type: "genum:robo", action: kind, payload }));
    } catch {
      /* ignore */
    }
  }
