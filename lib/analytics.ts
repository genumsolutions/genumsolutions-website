...
export async function getPageViewStats(options: { days?: number } = {}): Promise<{
  ...
  const allRows = (allResult.data ?? []) as {
    path: string;
    created_at: string;
    user_id: string | null;
  }[];
...
export async function getDashboardStats(): Promise<{
  ...
  const paidOrders = (paidOrdersRowsResult.data ?? []) as {
    total_npr: number;
    created_at: string;
  }[];
  const cartsAll = (cartsResult.data ?? []) as { lines: unknown }[];
  const cartItemCounts = cartsAll.map((cart) =>
    Array.isArray(cart.lines)
      ? cart.lines.reduce(
          (sum, line) => sum + (Number((line as { quantity?: number }).quantity) || 0),
          0
        )
      : 0
  );
