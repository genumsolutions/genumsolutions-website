...
export type ManagedCompanyInfo = {
  ...
  vatLabel: string;
  description: string;
  // C5 (2026-09-23): socials + WhatsApp (admin-editable).
...
export async function getManagedSettings(): Promise<SettingsBundle> {
  ...
  const [{ data: companyRow }, { data: programs }, { data: pilots }, { data: curricula }] =
    await Promise.all([
  ...
  const company: ManagedCompanyInfo = companyRow
    ? {
        ...
        shortName: companyRow.short_name ?? "",
        ...
        pan: companyRow.pan ?? "",
        vatLabel: companyRow.vat_label ?? "",
        description: companyRow.description ?? "",
        whatsappNumber: companyRow.whatsapp_number ?? "",
        facebookUrl: companyRow.facebook_url ?? "",
        instagramUrl: companyRow.instagram_url ?? "",
        tiktokUrl: companyRow.tiktok_url ?? "",
        linkedinUrl: companyRow.linkedin_url ?? "",
        youtubeUrl: companyRow.youtube_url ?? "",
      }
    : { ...emptyCompany };
...
export async function saveCompanyInfo(info: ManagedCompanyInfo): Promise<void> {
  ...
  await db.from("company_info").upsert({
    ...
    // C5: WhatsApp is normalized to digits-only on save (validators + wa.me
    // links both expect the canonical form).
...
export async function saveCurriculumHighlight(
  highlight: ManagedCurriculumHighlight
): Promise<void> {
  await createServiceClient()
