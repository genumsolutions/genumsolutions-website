...
const service = createClient(env.NEXT_PUBLIC_SUPABASE_URL || env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, {
...
try {
  ...
  const cu = await service.auth.admin.createUser({ email: CUST_EMAIL, password: PASSWORD, email_confirm: true })
  if (cu.error) throw new Error('createUser(customer): ' + cu.error.message)
  custId = cu.data.user.id
  const su = await service.auth.admin.createUser({ email: STAFF_EMAIL, password: PASSWORD, email_confirm: true })
  ...
  const { error: provErr } = await service.from('profiles').update({ role: 'staff' }).eq('id', staffId)
  ...
  assert('staff GET /api/admin/products (200)', listProducts.status === 200, `HTTP ${listProducts.status}`)

  const listOrders = await api('GET', '/api/admin/orders')
  assert('staff GET /api/admin/orders (200)', listOrders.status === 200, `HTTP ${listOrders.status}`)
  ...
  assert('staff PUT /api/admin/products (create probe, 200)', probeCreated, `HTTP ${createProbe.status}`)

  const probeEdit = await api('PUT', '/api/admin/products', {
    id: PROBE_ID, name: 'Staff Access Probe (edited)', category: 'Controllers & Boards', price: 1, stock: 2,
  })
  assert('staff PUT /api/admin/products (edit, 200)', probeEdit.status === 200, `HTTP ${probeEdit.status}`)

  const orderStatus = await api('PATCH', '/api/admin/orders', { id: '00000000-0000-0000-0000-000000000000', status: 'pending' })
  assert('staff PATCH /api/admin/orders (no-op order, 200 allowed path)', orderStatus.status === 200 || orderStatus.status === 404, `HTTP ${orderStatus.status}`)

  // Staff robot-settings listing is allowed; DELETE profile (robot) must be 403.
  const listRobots = await api('POST', '/api/admin/users', { action: 'listRobotSettings', userId: staffId })
  assert('staff POST /api/admin/users listRobotSettings (200)', listRobots.status === 200, `HTTP ${listRobots.status}`)

  const staffDeleteProduct = await api('DELETE', `/api/admin/products?id=${PROBE_ID}`)
  assert('staff DELETE /api/admin/products?id= -> 403', staffDeleteProduct.status === 403, `HTTP ${staffDeleteProduct.status}`)

  const staffRoleChange = await api('PATCH', '/api/admin/users', { userId: adminId, role: 'staff' })
  assert('staff PATCH /api/admin/users role-change -> 403', staffRoleChange.status === 403, `HTTP ${staffRoleChange.status}`)

  const staffRobotDelete = await api('POST', '/api/admin/users', { action: 'deleteRobotSetting', userId: adminId, robotId: 'nope' })
  assert('staff POST /api/admin/users deleteRobotSetting -> 403', staffRobotDelete.status === 403, `HTTP ${staffRobotDelete.status}`)
  ...
  assert('admin DELETE /api/admin/products?id= (probe removed, 200)', adminDeleteProbe.status === 200, `HTTP ${adminDeleteProbe.status}`)

  const adminDeleteUser = await api('DELETE', '/api/admin/users', { userId: custId })
  assert('admin DELETE /api/admin/users -> 403 (owner-only)', adminDeleteUser.status === 403, `HTTP ${adminDeleteUser.status}`)

  const adminJournalDelete = await api('DELETE', '/api/admin/journal?id=never-used-000000000000')
  assert('admin DELETE journal (no-op id, 200 allowed path)', adminJournalDelete.status === 200 || adminJournalDelete.status === 404, `HTTP ${adminJournalDelete.status}`)

  const adminRoleChange = await api('PATCH', '/api/admin/users', { userId: custId, role: 'customer' })
  assert('admin PATCH /api/admin/users role-change (200)', adminRoleChange.status === 200, `HTTP ${adminRoleChange.status}`)
  ...
  assert('customer GET /api/admin/products -> 401', custList.status === 401, `HTTP ${custList.status}`)

  const custDelete = await api('DELETE', `/api/admin/products?id=${PROBE_ID}`)
  assert('customer DELETE /api/admin/products?id= -> 401', custDelete.status === 401, `HTTP ${custDelete.status}`)

  const custUsers = await api('GET', '/api/admin/users')
  assert('customer GET /api/admin/users -> 401', custUsers.status === 401, `HTTP ${custUsers.status}`)
} catch (err) {
