...
try {
  const cu = await service.auth.admin.createUser({ email: CUST, password: PW, email_confirm: true })
  const au = await service.auth.admin.createUser({ email: ADMIN, password: PW, email_confirm: true })
  if (cu.error || au.error) throw new Error('createUser failed: ' + (cu.error?.message || au.error?.message))
  cid = cu.data.user.id
  aid = au.data.user.id
  ...
  assert('admin tier flip → pro (200 + persisted)', r.status === 200 && p1?.tier === 'pro', `HTTP ${r.status} tier=${p1?.tier}`)

  r = await callFn({ userId: cid, tier: 'free' }, adminJwt)
  const { data: p2 } = await service.from('profiles').select('tier').eq('id', cid).maybeSingle()
  assert('admin tier flip → free (200 + persisted)', r.status === 200 && p2?.tier === 'free', `HTTP ${r.status} tier=${p2?.tier}`)
