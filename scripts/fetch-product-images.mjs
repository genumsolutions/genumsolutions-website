...
const UA = {
  "User-Agent":
    "GenumSolutionsWebsite/1.0 (product catalog imagery; contact admin@genumsolutions.example)",
};
...
const products = [
  ...
  {
    id: "dc-geared-motor-pair",
    queries: ["Yellow DC gear motor TT", "TT gear motor robot", "geared DC motor yellow"],
  },
  { id: "sg90-servo-pair", queries: ["SG90 servo", "TowerPro SG90"] },
  { id: "ultrasonic-sensor", queries: ["HC-SR04 ultrasonic sensor", "HC-SR04"] },
  {
    id: "sensor-starter-pack",
    queries: ["Arduino sensor kit", "Arduino sensor shield pack", "sensor module kit Arduino"],
  },
  ...
  {
    id: "robot-wheel-set",
    queries: ["65mm robot wheel rubber", "robot wheel 65mm", "rubber wheel robot car"],
  },
  {
    id: "chassis-fastener-pack",
    queries: ["robot car chassis acrylic", "Arduino robot chassis kit", "acrylic robot chassis"],
  },
  {
    id: "dupont-jst-cable-pack",
    queries: ["Dupont wire jumper", "breadboard jumper wire", "Dupont cable"],
  },
  {
    id: "soldering-tool-kit",
    queries: ["soldering iron station electronics", "soldering workstation", "soldering iron kit"],
  },
  {
    id: "pla-filament",
    queries: ["PLA 3D printing filament", "PLA filament spool", "3D printer filament PLA"],
  },
  ...
  {
    id: "esp8266-board",
    queries: ["NodeMCU ESP8266 board", "ESP8266 NodeMCU", "ESP8266 development"],
  },
  ...
  {
    id: "bo-motor",
    queries: ["BO gear motor yellow", "TT gear motor 130", "yellow gearbox motor"],
  },
  {
    id: "hc06-bluetooth",
    queries: ["HC-06 Bluetooth module PCB", "HC-06 bluetooth", "HC-05 Bluetooth module"],
  },
  {
    id: "oled-display-13",
    queries: ["OLED display 1.3 inch I2C", "SH1106 OLED", "OLED display SSD1306"],
  },
  {
    id: "oled-display-09",
    queries: ["0.91 inch OLED display I2C", "OLED 0.96 display module", "SSD1306 OLED display"],
  },
  ...
  {
    id: "caster-wheel",
    queries: ["ball caster robot", "metal ball caster", "mini ball caster wheel"],
  },
  { id: "breadboard-jumper-set", queries: ["solderless breadboard", "breadboard"] },
  {
    id: "xt60-power-connectors",
    queries: ["XT60 connector yellow pair", "XT60 power connector", "XT60 battery connector"],
  },
...
async function main() {
  ...
  for (const r of results) {
    console.log(
      `${r.status === "ok" ? "OK  " : r.status === "exists" ? "SKIP" : "MISS"} ${r.id}${r.source ? `  <- ${r.source}` : r.error ? `  (${r.error})` : ""}`
    );
  }
