...
try {
  ...
  assert('staff list (200)', list.status === 200 && Array.isArray(list.data?.services), `status ${list.status}`)

  const created = await callFn(staff.token, { action: 'create', service: staffRow })
  assert('staff create (201)', created.status === 201, `status ${created.status} ${created.data?.error || ''}`)

  const { data: dbCheck } = await svc.from('services').select('*').eq('id', PROBE_ID)
  assert('row visible in DB', dbCheck?.length === 1 && dbCheck[0].category === 'Test & Tools', `rows ${dbCheck?.length}`)

  const updated = await callFn(staff.token, { action: 'update', service: { ...staffRow, name: 'Admin Services Probe v2' } })
  assert('staff update (200)', updated.status === 200, `status ${updated.status} ${updated.data?.error || ''}`)

  const toggled = await callFn(staff.token, { action: 'toggle', id: PROBE_ID, currentStatus: true })
  assert('staff toggle (200)', toggled.status === 200 && toggled.data?.active === false, `status ${toggled.status}`)
  ...
  assert('owner delete (200)', ownerDel.status === 200, `status ${ownerDel.status} ${ownerDel.data?.error || ''}`)
  ...
  const bad = await callFn(staff.token, { action: 'create', service: { id: 'Bad ID!', name: 'x' } })
  ...
  if (!KEEP) {
    try { await svc.from('services').delete().eq('id', PROBE_ID) } catch { /* ignore */ }
