...
try {
  ...
  assert('staff list (200)', list.status === 200 && Array.isArray(list.data?.products), `status ${list.status}`)
  ...
  const updated = await callFn(staff.token, { action: 'update', product: { ...staffRow, name: 'Admin Products Probe v2' } })
  ...
  const bad = await callFn(staff.token, { action: 'create', product: { id: 'Bad ID!', name: 'x' } })
  ...
  if (!KEEP) {
    try { await svc.from('products').delete().eq('id', PROBE_ID) } catch { /* ignore */ }
