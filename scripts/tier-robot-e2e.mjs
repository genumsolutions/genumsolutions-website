...
try {
  ...
  const { data: prof } = await service
    ...
    .maybeSingle();
  assert(
    "provisioning persisted — tier=pro role=admin",
    prof?.tier === "pro" && prof?.role === "admin",
    `tier=${prof?.tier} role=${prof?.role}`
  );
  ...
  assert(
    "admin /api/user/tier → pro",
    tierResp.status === 200 && tierResp.json?.tier === "pro",
    JSON.stringify(tierResp.json)
  );
  ...
  const put1 = await api("PUT", "/api/user/robot-settings", {
    ...
    settings: {
      maxSpeed: 220,
      steerTrim: 4,
      telemetryChannels: ["battery", "us_distance", "ir_line"],
      notes: "bench profile",
    },
  });
  ...
  assert(
    "admin GET own robots (2 rows, name-ordered)",
    own.status === 200 &&
      Array.isArray(own.json?.robots) &&
      own.json.robots.length === 2 &&
  ...
  const roundtripOk =
    row1 &&
    row1.settings.maxSpeed === 220 &&
    Array.isArray(row1.settings.telemetryChannels) &&
    row1.settings.telemetryChannels.length === 3;
  assert(
    "settings JSON round-trips (scalars + string arrays)",
    Boolean(roundtripOk),
    JSON.stringify(row1?.settings)
  );
  ...
  assert(
    "shape guard drops non-scalar/array junk, keeps valid keys",
    Boolean(junkOk),
    `kept keys: ${junkKeys.join(", ") || "(none)"}`
  );
  ...
  const xput = await api("PUT", "/api/user/robot-settings", {
    ...
    robotId: "robocar-2wd",
    robotName: "2WD Car v1.0.10",
    settings: { maxSpeed: 180, preset: "bench" },
  });
  assert(
    "admin cross-user PUT (writes customer row)",
    xput.status === 200,
    JSON.stringify(xput.json)
  );

  const xget = await api("GET", "/api/user/robot-settings?userId=" + customerId);
  assert(
    "admin cross-user GET (reads any user rows)",
  ...
  assert(
    "tier API rejects self-service writes (HTTP 405, GET-only route)",
    promote.status === 405,
    `HTTP ${promote.status}`
  );
  ...
  const del = await api(
    "DELETE",
    "/api/user/robot-settings?userId=" + adminId + "&robotId=drone-x1"
  );
  const afterDel = await api("GET", "/api/user/robot-settings");
  assert(
    "admin DELETE own robot row (gone on re-read)",
  ...
  assert(
    "admin signed out + customer signed in (tier flips pro→free)",
  ...
  assert(
    "customer sees the robot row the admin wrote for them",
  ...
  assert(
    "free customer PUT rejected by the pro gate (403 + message)",
  ...
  assert(
    "pro customer own-row PUT + persist (roundtrip)",
    proPut.status === 200 &&
      custGet.json?.robots?.length === 2 &&
  ...
  assert(
    "customer cross-user GET rejected (403)",
  ...
  assert(
    "customer cross-user PUT rejected (403)",
  ...
  assert(
    "signed-out GET rejected (401)",
    out2 === 200 && unauth.status === 401,
    `logout=${out2} HTTP ${unauth.status}`
  );
} catch (err) {
