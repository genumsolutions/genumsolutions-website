...
const SAMPLES = [
  {
    ...
    product: { category: '3D Models', price: 0, priceLabel: 'Request quote', stock: 0, description: 'Physics fidget toy that transforms from a spiral into a tree-like shape. Printed on demand.' },
  },
  {
    url: 'https://makerworld.com/en/models/732526',
    product: { category: '3D Models', price: 0, priceLabel: 'Request quote', stock: 0, description: 'Print-in-place 3-in-1 fidget clicker toy. Printed on demand.' },
  },
  {
    url: 'https://makerworld.com/en/models/559102',
    product: { category: '3D Models', price: 0, priceLabel: 'Request quote', stock: 0, description: 'Loud and clicky clockwork cog fidget toy, press-fit assembly. Printed on demand.' },
  },
]
...
let staffId
try {
  const { data, error } = await svc.auth.admin.createUser({ email: STAFF_EMAIL, password: PASSWORD, email_confirm: true })
  ...
  const { data: sess, error: signInError } = await anon.auth.signInWithPassword({ email: STAFF_EMAIL, password: PASSWORD })
  ...
  for (const s of SAMPLES) {
    ...
    console.log(`${ok ? 'IMPORTED' : 'FAILED'} (${r.status}) ${s.url.split('/').pop()} -> ${r.data?.product?.id || r.data?.error || '?'}`)
  }
} catch (e) {
  console.log('THREW', e.message)
} finally {
  if (staffId) {
    try { await svc.auth.admin.deleteUser(staffId) } catch { /* ignore */ }
  }
}