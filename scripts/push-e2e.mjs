...
try {
  ...
  page.on("requestfailed", (r) => {
    if (r.url().includes("/api/auth/login"))
      console.log(`[net] login request FAILED: ${r.failure() && r.failure().errorText}`);
  });
  ...
  try {
    ...
    throw new Error(
      "sign-in form not found on /account — dumping selectors: " +
        (await page.evaluate(() =>
          Array.from(document.querySelectorAll("button,input"))
            .map((el) => el.tagName + ":" + (el.name || el.textContent || "").trim().slice(0, 30))
            .join(" | ")
        ))
    );
  }
  ...
  try {
    ...
    await page.waitForFunction(() => document.body.innerText.includes("Order notifications"), {
      timeout: 45000,
      polling: 500,
    });
  } catch {
    ...
    const stillForm = await page.evaluate(() =>
      Boolean(document.querySelector('input[name="password"]'))
    );
    throw new Error(
      `sign-in did not reach the account view (form still present: ${stillForm}). page says: ${bodyText}`
    );
  }
  ...
  try {
    await page.waitForFunction(() => document.body.innerText.includes("Turn off on this device"), {
      timeout: 45000,
      polling: 500,
    });
  } catch {
  ...
  assert(
    "PushManager subscription exists",
    Boolean(endpoint),
    endpoint ? "endpoint host: " + new URL(endpoint).host : "none"
  );
  ...
  assert(
    "subscription persisted in web_push_subscriptions (RLS row)",
    rowMatch,
    `${(subRows || []).length} row(s) for user`
  );

  // ---- 5. real order + real status change (trigger path) ----------------------
  const ord = await service
    .from("orders")
    .insert({
      user_id: userId,
  ...
  await page.evaluate(() => {
    ...
    navigator.serviceWorker.addEventListener("message", (event) => {
      if (event.data && event.data.type === "PUSH_RECEIVED")
        window.__pushArrived = event.data.payload;
    });
  });
  ...
  assert(
    "REAL push arrived in the browser service worker",
    Boolean(payload),
    payload ? `title="${payload.title}" body="${payload.body}"` : "not detected within 20s"
  );
  const contentOk = payload && /rover|payment/i.test((payload.title || "") + (payload.body || ""));
  assert(
    "payload matches the order-status message",
    Boolean(contentOk),
