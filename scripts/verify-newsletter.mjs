...
try {
  ...
  check('row exists with source=footer', found.length === 1 && found[0].source === 'footer',
    `rows ${found.length}`)
  ...
  found = await rows()
  check('still exactly ONE row (case-insensitive dedupe)', found.length === 1, `rows ${found.length}`)
  check('source refreshed to checkout', found[0]?.source === 'checkout', `source ${found[0]?.source}`)
  ...
  const { error: delErr } = await service.from('newsletter_subscribers').delete().eq('email', EMAIL)
