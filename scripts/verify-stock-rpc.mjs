...
try {
  ...
  assert(
    "decrement subtracts ordered quantity",
    (await stockOf(PRODUCT_ID)) === 3,
    `stock=${await stockOf(PRODUCT_ID)}`
  );

  // 1b. clamp: decrement 99 on stock 3 -> 0 (never negative)
  await staffDb.rpc("adjust_order_stock", {
    order_items: [{ productId: PRODUCT_ID, name: "x", price: 1, quantity: 99 }],
    direction: "decrement",
  });
  assert(
    "decrement clamps at zero",
    (await stockOf(PRODUCT_ID)) === 0,
    `stock=${await stockOf(PRODUCT_ID)}`
  );
  ...
  assert(
    "restore adds back",
    (await stockOf(PRODUCT_ID)) === 2,
    `stock=${await stockOf(PRODUCT_ID)}`
  );

  // 3. bad direction rejected
  const bad = await staffDb.rpc("adjust_order_stock", {
    order_items: items,
    direction: "sideways",
  });
  ...
  // 4. customer JWT rejected (staff gate)
  const asCust = await custDb.rpc("adjust_order_stock", {
    order_items: items,
    direction: "restore",
  });
  assert(
    "customer rejected (staff gate)",
    Boolean(asCust.error),
    asCust.error?.message || "no error"
  );
  ...
  assert(
    "mark_order_paid transition (true)",
    pay1.data === true,
    `got ${pay1.data} ${pay1.error?.message || ""}`
  );
  assert(
    "paid order decremented stock",
    (await stockOf(PRODUCT_ID)) === 0,
    `stock=${await stockOf(PRODUCT_ID)}`
  );
  const { data: paidRow } = await svc
    .from("orders")
    .select("status")
    .eq("id", orderId)
    .maybeSingle();
  ...
  assert(
    "re-run did not double-decrement",
    (await stockOf(PRODUCT_ID)) === 0,
    `stock=${await stockOf(PRODUCT_ID)}`
  );

  // 7. missing order -> false (no throw)
  const pay3 = await staffDb.rpc("mark_order_paid", {
    order_id: "00000000-0000-4000-8000-000000000000",
  });
  assert(
    "mark_order_paid missing order (false)",
    pay3.data === false && !pay3.error,
    `got ${pay3.data}`
  );

  // --- restore_order_stock ------------------------------------------------
  // 8. paid -> cancelled: restores 2 AND flips the order to cancelled (atomic)
  const res1 = await staffDb.rpc("restore_order_stock", {
    order_id: orderId,
    expect_status: "paid",
  });
  const { data: cancelledRow } = await svc
    .from("orders")
    .select("status")
    .eq("id", orderId)
    .maybeSingle();
  assert(
    "restore on cancel restores stock",
    res1.data === 1 && (await stockOf(PRODUCT_ID)) === 2,
    `rows=${res1.data} stock=${await stockOf(PRODUCT_ID)}`
  );
  assert(
    "restore flipped order to cancelled",
  ...
  const res2 = await staffDb.rpc("restore_order_stock", {
    order_id: orderId,
    expect_status: "paid",
  });
  assert(
    "restore re-run no-ops",
    res2.data === 0 && (await stockOf(PRODUCT_ID)) === 2,
    `rows=${res2.data} stock=${await stockOf(PRODUCT_ID)}`
  );

  // 10. expect_status=paid on a never-paid pending order -> no-op
  const { data: ord2 } = await svc
    .from("orders")
    .insert({
      user_id: customer.id,
      items,
    ...
    .select()
    .single();
  const res3 = await staffDb.rpc("restore_order_stock", {
    order_id: ord2.id,
    expect_status: "paid",
  });
  assert(
    "restore on pending order no-ops",
    res3.data === 0 && (await stockOf(PRODUCT_ID)) === 2,
    `rows=${res3.data} stock=${await stockOf(PRODUCT_ID)}`
  );
  ...
  if (!KEEP) {
    try {
      await svc.from("orders").delete().eq("email", CUSTOMER_EMAIL);
    } catch {
      /* ignore */
    }
    try {
      await svc.from("products").delete().eq("id", PRODUCT_ID);
    } catch {
      /* ignore */
    }
