...
try {
  ...
  async function login(p, email) {
    ...
    if (status !== 200 || session?.user?.email !== email) {
      throw new Error(
        `login failed for ${email} (api=${status}, session=${JSON.stringify(session).slice(0, 140)})`
      );
    }
    return session.user;
  }
  ...
  // ================= SECTION 1 — storefront + unification =================
  {
    ...
    report(
      1,
      "Home renders curriculum highlights from shared tables",
      hasCurriculum ? "PASS" : "FAIL",
  ...
  }
  {
    ...
    report(
      1,
      "/tools fleet catalogue: 9 MODE n/9 chips",
      chips >= 9 ? "PASS" : "FAIL",
      `${chips} chips found`
    );
    report(
      1,
      "/tools D-1 parked note readable",
      /parked|remote control/i.test(fleet) ? "PASS" : "SNAG",
      "visual wording check recommended"
    );
  }
  {
  ...
  }
  {
    ...
    report(
      ...
      orderOk ? "PASS" : "SNAG",
      `first tabs: ${tabs.slice(0, 3).join(" → ")}`
    );
    // Stats load async — wait for the dashboard fetch to paint its cards.
    try {
      await page.waitForFunction(() => /transactions/i.test(document.body.innerText), {
        timeout: 20000,
        polling: 500,
      });
      ...
      report(
        1,
        "Dashboard shows the Transactions card",
        "FAIL",
        "card text never rendered within 20s"
      );
    }
  }
  {
    ...
    report(
      1,
      "P2 gate: admin curriculum edit shows on home",
      created === 200 && visible ? "PASS" : "SNAG",
      created !== 200
        ? `settings POST → ${created}`
        : visible
          ? "probe line rendered"
          : "probe line not on home"
    );
    // Probe cleanup is done server-side in the finally block (by age_band),
    // which is robust regardless of the GET response shape.
    report(
      1,
      "Curriculum probe written via admin API (cleanup in finally)",
      created === 200 ? "PASS" : "SNAG",
      `PUT → ${created}`
    );
  }
  ...
  // ================= SECTION 2 — W-6 dark mode =================
  {
    ...
    const seqOk =
      l0 &&
      /Theme: (light|dim)/.test(l0) &&
      cycle[0] &&
      /Theme: (light|dim)/.test(cycle[0]) &&
      cycle[0] !== l0 &&
      cycle[1] === l0;
    report(
      2,
      "Toggle cycles light ⇄ dim (System removed)",
      seqOk ? "PASS" : "SNAG",
      `start="${l0}" then="${cycle
        .map((c) => (c || "").slice(7, 20))
        .join('", "')
  ...
  }
  {
    ...
    report(
      2,
      "2-mode migration: legacy system → dim, fresh (no stored) → light",
  ...
  }
  {
    ...
    report(
      2,
      "No-flash: pre-paint script present + dim applied on reload",
  ...
  }
  {
    ...
    for (const path of pagesToWalk) {
      ...
      if (path === "/admin") {
        ...
        await page
          .waitForFunction(() => /transactions/i.test(document.body.innerText), {
            timeout: 20000,
            polling: 500,
          })
          .catch(() => undefined);
      ...
      const sample = await page.evaluate(() => {
        ...
        function parse(c) {
          ...
          return m ? [+m[1], +m[2], +m[3], m[4] === undefined ? 1 : +m[4]] : [255, 255, 255, 1];
        }
        function bgOf(el) {
          let node = el;
        ...
        for (const el of document.querySelectorAll("body *")) {
          ...
          const fg =
            a < 1
              ? fgRaw.slice(0, 3).map((v, i) => Math.round(v * a + bg[i] * (1 - a)))
              : fgRaw.slice(0, 3);
          const L1 = lum(...fg),
            L2 = lum(bg[0], bg[1], bg[2]);
      ...
      if (process.env.E2E_DUMP_CONTRAST && sample.fails.length) {
        ...
        for (const f of sample.fails)
          console.log(
            `     ratio ${String(f.ratio).padEnd(5)} <${f.tag} class="${f.cls}"> color=${f.color} bg=${f.bg} text="${f.text}"`
          );
      }
      walkNotes.push(`${path}: ${sample.hard} hard / ${sample.warn} soft of ${sample.checked}`);
      if (sample.hard > worst.violations)
        worst = { page: path, violations: sample.hard, warns: sample.warn, theme: sample.theme };
    }
    const allDim = true; // sampler reported data-theme per page inside notes only; global verdict from worst
    report(
      2,
      "Dim walk: no unreadable text on home/products/tools/account/checkout/admin",
      worst.violations === 0 ? "PASS" : "SNAG",
      walkNotes.join(" · ")
    );
  }
  {
    ...
    report(
      2,
      "Theme preference saved to profiles (Supabase shared truth)",
    ...
    report(
      ...
      afterReload === expected ? "PASS" : "SNAG",
  ...
  // ================= SECTION 3 — regression smoke =================
  {
    ...
    report(
      3,
      "Header cart badge counts items",
      /Open checkout, 1 item/.test(badge || "") ? "PASS" : "SNAG",
      `aria="${badge}"`
    );
  }
  {
    ...
    report(
      3,
      "Cart → checkout → COD order places cleanly",
      placed.status === 200 && orderId ? "PASS" : "FAIL",
  ...
  }
  {
    ...
    report(
      3,
      "Sign out works (avatar menu → Log out)",
      clicked && sessionAfter?.user === null ? "PASS" : "SNAG",
  ...
  }
  {
    ...
    const mobile = await page.evaluate(() => ({
      ...
      buttons: [...document.querySelectorAll("header button")].filter(
        (b) => b.getBoundingClientRect().right <= window.innerWidth + 1
      ).length,
    }));
    report(
      3,
      "Mobile (390px): header fits, no horizontal overflow",
  ...
  // ================= SECTION 4 — W-3 push (live) =================
  {
    ...
    page.on("response", (r) => {
      if (r.url().includes("/api/push/subscribe")) subscribeSettle(r.status());
    });
    ...
    for (let i = 0; i < 40 && !cardShown; i++) {
      cardShown = await page
        .evaluate(() => document.body.innerText.includes("Order notifications"))
        .catch(() => false);
    ...
    await page.evaluate(() => {
      const b = [...document.querySelectorAll("button")].find((x) =>
        x.textContent.includes("Turn on notifications")
      );
      if (b) b.click();
      return Boolean(b);
    });
    try {
      await page.waitForFunction(
        () => document.body.innerText.includes("Turn off on this device"),
    ...
    report(
      ...
      (subs || []).length === 1 && subStatus === 200 ? "PASS" : "FAIL",
      `rows=${(subs || []).length}, api=${subStatus}`
    );
    ...
    if (orderId) {
      ...
      if (custOrder.status !== 200 || !custOrder.id) {
        report(
          4,
          "Live fire: customer order for the push test",
          "FAIL",
          `POST /api/orders → ${custOrder.status}, id=${custOrder.id}`
        );
      } else {
        ...
        report(
          4,
          "Live fire: admin status change accepted",
          patch === 200 ? "PASS" : "FAIL",
          `PATCH → ${patch}`
        );
      ...
      report(
        4,
        "Notification pops on the subscribed browser",
      ...
      await page.evaluate(() => {
        const b = [...document.querySelectorAll("button")].find((x) =>
          x.textContent.includes("Turn off on this device")
        );
        if (b) b.click();
        return Boolean(b);
      });
    ...
    const prodForCheckout = await service
      .from("products")
      .select("id")
      .eq("active", true)
      .gt("stock", 0)
    ...
    let optIn = 0;
    for (let i = 0; i < 80; i++) {
      optIn = await page.evaluate(
        () => (document.body.innerText.match(/Want a push notification/g) || []).length
      );
      if (optIn > 0) break;
      await sleep(500);
    }
    report(
      4,
      "Checkout opt-in appears (once)",
      optIn === 1 ? "PASS" : "SNAG",
      `${optIn} occurrence(s)`
    );
    await page.evaluate(() => {
      const b = [...document.querySelectorAll("button")].find(
        (x) => x.textContent.trim() === "Not now"
      );
      if (b) b.click();
      return Boolean(b);
    });
    ...
    let optInAfter = 0;
    for (let i = 0; i < 12; i++) {
      optInAfter = await page.evaluate(
        () => (document.body.innerText.match(/Want a push notification/g) || []).length
      );
    ...
    report(
      4,
      '"Not now" makes the opt-in stay away',
  ...
  for (const id of [orderId, customerOrderId].filter(Boolean)) {
    await page
      .evaluate(async (oid) => {
        await fetch(`/api/admin/orders?id=${oid}`, { method: "DELETE" });
      }, id)
      .catch(() => undefined);
  }
