...
try {
  ...
  assert('makerworld preview (200)', prev.status === 200 && prev.data?.preview, `status ${prev.status}`)
  ...
  assert('makerworld create (201)', created.status === 201, `status ${created.status} ${created.data?.error || ''}`)
